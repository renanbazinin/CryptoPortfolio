  ...
  const handleRemoveTransaction = async (transactionId: string) => {
    try {
      ...
      if (portfolioId.length === 24 && /^[0-9a-fA-F]{24}$/.test(portfolioId)) {
        endpoint = `${API_BASE_URL}/api/portfolio/${portfolioId}/coins/${coin.coinId}/transactions/${transactionId}`;
      } else {
        endpoint = `${API_BASE_URL}/api/portfolio/byUserId/${portfolioId}/coins/${coin.coinId}/transactions/${transactionId}`;
      }
